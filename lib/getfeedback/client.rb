module Getfeedback
  class Client
    DEFAULT_API_BASE_URL = "https://api.getfeedback.com"
    DEFAULT_HTTP_ADAPTER = HTTPAdapter.new

    def initialize(opts = {})
      @api_key = opts[:api_key] or raise ArgumentError, "You must provide an API key by setting Getfeedback.api_key = '123abc' or passing { :api_key => '123abc' } when instantiating Getfeedback::Client.new"
  ...
  private
    ...
    def default_headers
      @default_headers ||= {
        ...
        'User-Agent' => "Getfeedback RubyGem #{Getfeedback::VERSION}"
      }.freeze
    end
  end
end
